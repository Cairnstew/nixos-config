...
in
{
  isoSettingsSubmodule = types.submodule {
    options = {
      baseModule = mkOption {
        type = types.enum [ "minimal" "graphical" "graphical-kde" "graphical-combined" ];
        default = "minimal";
      ...
      tailscale = {
        ...
        authKeyEncryptedSource = mkOption {
          type = types.nullOr types.path;
          default = null;
          description = ''
            Path to the encrypted tailscale auth key (.age file) to embed in the ISO.
            When set, a decrypt service will run at boot using the embedded age private
            key to produce the plaintext key at tailscale.authKeyFile before
            tailscale-autoconnect runs.
          '';
        };
      };
