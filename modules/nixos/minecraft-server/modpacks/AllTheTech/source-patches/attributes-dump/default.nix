# attributes-dump — temporary diagnostic mod that dumps all registered entity
# base attribute values to attributes-dump.json, then exits the JVM.
#
# NOT a permanent pack mod — built and run once during attributes-regenerate,
...
buildModSource {
  name = "attributes-dump-1.0.0.jar";
  src = ./src;
  patches = [ ];
  ...
  buildCmd = ''
    ...
    if [ -z "$jar" ]; then
      echo "attributes-dump: no playable jar found under build/libs/" >&2
      exit 1
    fi
    cp "$jar" "$out"
  '';
  outputHash = "sha256-uGoCv9Qpuld3tVkoOZJ+3Po9rmczd8DAETWihK8Zej8=";
}
