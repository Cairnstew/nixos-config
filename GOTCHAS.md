...
- **Preferred:** Use `just deploy-with-keys <hostname> <ip>` *instead of* `just deploy` for the initial install. This pre-generates the host key, registers it in `modules/nixos/common.nix` (`agenixManager.keys.systems`), rekeys secrets, and deploys with `--extra-files` — the target boots with the correct key and agenix works immediately.
- **Existing workaround (post-deploy):** Fetch the new host key, add it to `modules/nixos/common.nix` under `agenixManager.keys.systems`, run `agenix-manager rekey` to re-encrypt all secrets, then rebuild. This is only needed once per machine, and only if you didn't use `deploy-with-keys`.
