...
**Standalone jupyter/python flakes (outside nixos-config) must reuse the nixos-config overlay or inline-snapshot breaks the build**

Symptom (2026-09-23, Obsidian vault `Software_Development_for_Data_Science/flake.nix`): building a fresh devShell/python env with jupyter from bare `nixos-unstable` (python312) fails with `inline-snapshot-0.34.2` test failures (3 failed, 1402 passed) in the Nix sandbox; cascades through `fastapi → openapi-core → jupyterlab-server → jupyter`. The main config already fixes this in `overlays/default.nix:88-98` (`inline-snapshot` with `doCheck = false`), but the vault flake inherits nixpkgs directly so the fix didn't apply. Fix (this run): add `nixos-config` as a flake input (`git+file:///home/seanc/nixos-config`) and pass `overlays = [ inputs.nixos-config.overlays.default ]` when importing nixpkgs in `perSystem`. Verified: env builds and `import numpy, pandas, scipy, sklearn, matplotlib, jupyter` all succeed. Lesson: any standalone flake that builds a jupyter-bearing python env must inherit the nixos-config overlay (or replicate `doCheck = false` for inline-snapshot) — the fix exists in exactly one place and won't propagate otherwise.
---

**Stylix app targets clobber custom home-module configs (attrs options don't merge across priorities) — and Stylix's zed theme is invalid**

Symptom (2026-09-23, laptop): `~/.config/zed/settings.json` contained only 6 keys with `"theme": "Base16 untitled"` while the custom zed module (`modules/home/zed-editor/config.nix`) generates ~30 keys incl. `theme = "catppuccin-mocha"`. Zed showed a light/white theme. Two-fold cause: (1) Stylix's zed target (`stylix/modules/zed/hm.nix`) sets `programs.zed-editor.userSettings` at normal priority, while the repo module sets it with `lib.mkDefault` — for attrs options the module system keeps ONLY the highest-priority definition (no cross-priority merge), so Stylix replaced the module's whole settings. Any Stylix target whose option a custom home module also sets with mkDefault will clobber it the same way. Fix: disable the target via `home-manager.sharedModules` (option lives in Stylix's HM module, not the NixOS module): `{ stylix.targets.zed.enable = lib.mkDefault false; }` is in `modules/nixos/stylix/config.nix` alongside the existing firefox sharedModules entry. (2) Stylix's generated zed theme is invalid because its zed template interpolates `{{scheme-name}}`/`{{scheme-variant}}` from the base16Scheme, and the repo's base16Scheme provides only `slug`+baseXX — base16.nix defaults name→"untitled", variant→"unspecified", which Zed's schema rejects (requires light/dark) → falls back to the default LIGHT theme. If you re-enable Stylix zed, first add `name` and `variant` to the base16Scheme in `modules/nixos/stylix/config.nix`. Lesson: a change that enables the Stylix target for an app that already has a custom module = wiped settings + possibly invalid theme; check after rebuild via `cat ~/.config/<app>/settings.json`.
