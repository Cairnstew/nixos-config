...
Symptom (2026-08-15): `minecraft-server-test.service` crash-looped with `java.lang.NoClassDefFoundError: org/lwjgl/Version` at boot, thrown from `net.caffeinemc.mods.sodium.client.compatibility.checks.PreLaunchChecks`. Cause: `packwizSymlinks` (`modules/nixos/minecraft-server/config.nix`) symlinked **every** mod in `checksums.json` into the server's `mods/` — including client-only render/GL mods (`side = "client"` in their `<pack>/mods/<name>.pw.toml`, e.g. Sodium, Iris, Continuity, Entity Model Features, 47 of 254 in testModpack). Those mods' pre-launch checks require LWJGL (a client graphics library), which a headless server never ships → guaranteed boot crash. packwiz itself skips `side = "client"` mods when installing a pack to a server; our Nix path had no equivalent. Fix: filter `mods` by `side` before linking — `serverMods = lib.filterAttrs (name: _: !(fromTOML (readFile "${srv.packwiz}/mods/${name}")).side or "both" == "client") mods`, then `mkModLinks serverMods`. Mods with no `side` field default to `both` and are kept. Note: this only clears the client-mod crash stage — a pack with deeper server-incompatible mods (e.g. Sinytra Connector/Preloading Tricks failing on `org.apache.commons.compress.utils.Lists`, removed in commons-compress 1.27) can still fail later; that's a separate modpack-compat issue.
