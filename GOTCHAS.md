...
Symptom: overriding an option with a plain assignment silently loses to a `lib.mkForce` elsewhere, or adding a host-level assignment conflicts with one. Cause: mkForce sites beyond the documented ones. Verified current sites: modules/nixos/tailscale/config.nix:24 (`environment.etc."resolv.conf".source = lib.mkForce`), modules/nixos/bluetooth.nix:37 (`CapabilityBoundingSet`), configurations/nixos/desktop/default.nix:26-44 (my.vm.extraConfig VM-variant overrides), modules/nixos/ai/comfyui/config.nix:92,112,113, modules/nixos/homeManager/config.nix:176-184 (agenix owner/group), modules/nixos/disko/config.nix:111,116, modules/flake-parts/packages.nix:23-24, modules/flake-parts/test-runner.nix:15-16, modules/flake-parts/vm/config.nix:44-45, modules/flake-parts/live-iso/config.nix:87,102, modules/nixos/common.nix:205,225 (system.activationScripts.agenixManagerSecretsNix .text/.deps). Fix: grep `mkForce` in modules/ + configurations/ before overriding; match with `lib.mkForce` or gate behind the same mkIf.
