...
- **Preferred:** Use `just deploy-with-keys <hostname> <ip>` *instead of* `just deploy` for the initial install. This pre-generates the host key, registers it in `secrets/secrets.nix`, rekeys secrets, and deploys with `--extra-files` — the target boots with the correct key and agenix works immediately.
- **Existing workaround (post-deploy):** Run `just register-host <hostname> <ip>` to fetch the new host key, add it to `secrets/secrets.nix`, run `agenix -r` to re-encrypt all secrets, then rebuild. This is only needed once per machine, and only if you didn't use `deploy-with-keys`.
