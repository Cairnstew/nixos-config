...
**Passing an interpolated flake-input path string to a `types.either types.lines types.path` option installs the path AS TEXT — an 85-byte stub file whose whole content is a bare store path, not a symlink**

Symptom (2026-09-24): `opencode` failed with `AggregateError: 4 errors building "/nix/store/kfff9mh7…-hm_opencodetoolspackwizjarmeta.ts"` — `Invalid flag "t"/"o"/"r"/"e"` at lines 1:8–1:11. The "tool file" in the store was an 85-byte **regular file whose entire content was `/nix/store/nkd2mndm…-source/opencode/tools/packwiz-jar-meta.ts`** (a bare store-path string). Cause: the option `my.programs.opencode.tools` is declared `type = types.attrsOf (types.either types.lines types.path)` (`modules/home/opencode/options.nix:727`), and the wiring passed a string-interpolated path `"${input}/opencode/tools/x.ts"` — the upstream home-manager opencode module renders each `tools` entry via `if lib.isPath content then { source = content; } else { text = content; }`, and a **string** (any string, including an interpolated input path) takes the `text` branch: the file is written as literal content. The result, `/nix/store/<hash>-source/opencode/tools/x.ts`, is then *parsed as TypeScript*: Bun read `/nix/` as a regex literal and `store/...` as regex flags → 4 `Invalid flag` errors. Also verified on this Nix 2.34.8: `builtins.path`, `builtins.toPath`, and a flake input's `outPath` all evaluate as **string** (not path), and only `/. + builtins.unsafeDiscardStringContext str` yields a genuine path that reaches the `source` branch. Fix: always pass a real path (e.g. `/. + builtins.unsafeDiscardStringContext (input.outPath + "/tools/x.ts")`) to any `either lines path` option — never a string-interpolated input path; and for opencode, external-repo tool config no longer belongs in `~/.config/opencode` at all (repos own their `.opencode/`, see AGENTS.md §11).
> **RESOLVED — 2026-09-24 — the offending MC opencode wiring was removed from nixos-config entirely (tools moved to `nixos-minecraft-modpacks/.opencode/tools/`; all 34 build cleanly with `bun build`, and four were exercised with real calls: packwiz-jar-meta, packwiz-ore, mc-pack-status, packwiz-analyze); the `tools` option's string-vs-path trap stays as the lesson.**

**NixOS-declared user services restart on `nixos-rebuild switch` when `restartTriggers` change — use this for config-file-driven bars/daemons instead of manual restarts or path-unit hacks**

Symptom (2026-09-24): every waybar config change required a manual `systemctl --user restart waybar`; a rebuild rewrote `/etc/xdg/waybar/config` but the running bar kept the old config (waybar only reads it at start). Fix (`modules/nixos/hyprland/bar/config.nix:326`): declare the user unit in a **NixOS** module (as this repo already does — `systemd.user.services.waybar` in the hyprland bar module) and set `restartTriggers = [ (builtins.hashString "sha256" (configJSON + styleCSS)) ]`; the hash differs only when the generated files change, so waybar restarts exactly on config changes and never on unchanged rebuilds. Verified against the pinned nixpkgs (`9ae611a`): the Rust switch-to-configuration runs a per-user child (`do_user_switch`) that compares `/etc/systemd/user` units old-vs-new and restarts units whose X-Restart-Triggers changed — **this applies to NixOS-declared units only; units shadowed by `~/.config/systemd/user` (i.e. home-manager ones) are explicitly skipped**, so a config that needs switch-time restarts must be declared in a NixOS module, not via home-manager `systemd.user.services`.

**Waybar custom-module "button panes": use the native GtkMenu popup (`menu`/`menu-file`/`menu-actions`), NOT hover-div HTML — waybar cannot dispatch clicks to separate elements inside a module label**

Symptom (2026-09-24): wanted an interactable control pane (Previous/Play-Pause/Next) on the Spotify widget. A CSS `:hover` div inside the label *renders* fine but every click in it fires the module's single `on-click` — there is no per-element click dispatch in waybar label HTML. Fix (`configurations/nixos/desktop/default.nix:240`): waybar's GtkMenu popup is the native "pane with buttons" — set `menu = "<event>"` (e.g. `"on-click-right"`), `menu-file = <path to GtkBuilder XML with a GtkMenu id="menu">`, and `menu-actions` mapping each GtkMenuItem `id` to a command (`~` in menu-file is expanded; menu popup items each run their own command, with the same restricted-PATH caveat as on-click → use absolute paths). The XML ships from the owning home module (`modules/home/spotify/config.nix` `home.file.".config/waybar/spotify-menu.xml"`); the menu is themeable via waybar CSS `menu { }` / `menuitem { }` / `menuitem:hover`. Note the `menu-actions` commands receive no arguments and the widget re-executes after an action when `exec-on-event` is true (default), so state refreshes automatically.
GOTCHA INSIDE THE FIX: `my.desktop.hyprland.bar.customModules.<name>` is a CLOSED option submodule — the first activation fails with "The option `…customModules.<name>.menu' does not exist" until `menu`/`menu-file`/`menu-actions` are declared in `modules/nixos/hyprland/bar/options.nix` (same for any future pass-through key, e.g. `signal`, `restart-interval`); the submodule uses literal waybar names (`menu-file`, `menu-actions`), so no camelCase→kebab translation applies outside `returnType`.
SUPERSEDED in the file delivery detail (2026-09-24): do NOT ship the menu XML via the home module's `home.file` — home-manager activation runs after waybar's one-shot menu build; use `bar.menuFiles` (system `/etc/xdg/waybar`), see the entry below.

**Spotify widget playback control via the Web API is scope- AND plan-gated — `playerctl` routes to whatever player is active, not just Spotify**

Symptom (2026-09-24): the media-control buttons (⏮ ⏯ ⏭) were wired to `playerctl previous|play-pause|next`, which follows the *most recently active* player — with Firefox/YouTube playing, the buttons control the wrong thing. Also, `spotipy.SpotifyPKCE` refreshes a cached token silently but **scope is fixed at authorization time**, so adding `user-modify-playback-state` to `SCOPE` in `now-playing-waybar.py:47` does not upgrade an existing token: control calls fail with HTTP 403 "Insufficient scope" until the user re-runs `spotify-now-playing --auth` once. And unlike playback *reads* (free tier), Web API playback *commands* (`previous_track`/`next_track`/`start_playback`/`pause_playback`) return `PREMIUM_REQUIRED` on free accounts (`spotify-click.py:119`) — that's an API constraint, not a bug. Fix: control buttons call the Web API player endpoints (`previous`→`previous_track`, `toggle`→`current_playback().is_playing ? pause : start`, `next`→`next_track`) so they act only on the Spotify account's active device; `pkgs.playerctl` dropped from the click wrapper's `runtimeInputs` (`modules/home/spotify/config.nix`).

**`my.desktop.hyprland.bar.customModules` render sorted ALPHABETICALLY per side — multi-module clusters need the `order` option; and waybar's GtkMenu is vertical-only, so a horizontal button row = separate static modules**

Symptom (2026-09-24): adding three inline media buttons (⏮ ⏯ ⏭) next to the song widget, names `spotify-*` sorted `spotify-next` before `spotify-now-playing` — the row would have rendered in the wrong sequence. `customModules` is an attrset, so a side's list is `builtins.attrNames` (lexicographic); insert order is not preserved. Fix: the custom-module submodule gained an `order` int option (`modules/nixos/hyprland/bar/options.nix:100`) — per side, sort by `order` then name; related modules use adjacent values (info=0, back=1, play=2, forward=3 in `configurations/nixos/desktop/default.nix`). Corollary for layouts the popup can't express: GtkMenu has no horizontal mode, so "buttons left to right" means one static custom module per button (`format` only, no exec → static text; own `on-click` each; `tooltip = false`), styled as a cluster in CSS.

**Waybar `menu-file` must be a SYSTEM path (`/etc/xdg/waybar/…` via `bar.menuFiles`), NOT `~/.config/waybar` via home-manager — HM user activation runs AFTER the switch restarts units**

Symptom (2026-09-24): after wiring the Spotify menu popup through a home-manager file (`home.file.".config/waybar/spotify-menu.xml"`), every rebuild worked but the menu was dead: waybar journal at startup said `Error while creating the menu : Failed to expand file: ~/.config/waybar/spotify-menu.xml. Menu popup not activated.`, and every right-click spammed `gtk_menu_popup_at_pointer: assertion 'GTK_IS_MENU (menu)' failed` (the menu was never built). Root cause: on `switch`, systemd user units whose `restartTriggers` changed restart in the NixOS activation phase (13:25:50 in this session) BEFORE the home-manager user activation creates `~/.config/waybar/spotify-menu.xml` (13:25:51); waybar builds its popup menu ONCE at startup and never re-reads the file. Fix (`modules/nixos/hyprland/bar/options.nix:136`, `configurations/nixos/desktop/default.nix:243`): the bar module gained a `menuFiles` option that writes each entry to `/etc/xdg/waybar/<name>` via `environment.etc` — system activation materialises it before any unit restart, so waybar finds it. The menu-file hash is folded into the waybar `restartTriggers` hash so menu changes also trigger a restart. General rule: ANY file waybar must read at startup belongs under `/etc/xdg/waybar` (generated by the bar module), never a home-manager path.

**Python `webbrowser.open` under a restricted PATH "succeeds" by spawning xdg-open, which can't find any browser — probe home-profile browser paths FIRST, not as a fallback**

Symptom (2026-09-24): the Spotify `--auth` flow printed its authorize URL and "Waiting for the callback…", then nothing opened; stderr showed a wall of `xdg-open: line 1032: firefox: command not found` … `no method available for opening 'https://accounts.spotify.com/authorize?...'`. Cause: the flow runs inside a terminal launched from the waybar widget, inheriting a PATH with no profile dir; `webbrowser.open(url)` still returns `True` because Python's webbrowser just `Popen`s the `xdg-open` fallback — which then fails internally, so a profile-path fallback after `webbrowser.open` never runs. Fix (`modules/home/spotify/now-playing-waybar.py:110`): probe absolute profile-path browsers first (`/etc/profiles/per-user/<user>/bin/{firefox,brave,...}`), launch the first that exists, and only fall back to `webbrowser`. Same root as the waybar unit-PATH entry above: any command run from a waybar-launched process must use absolute paths.

**Waybar custom modules can't find bare `exec`/`on-click` commands — the systemd user unit PATH excludes the home profile bin and system PATH**

Symptom (2026-09-24, desktop): the Spotify now-playing widget never appears; `journalctl --user -u waybar` spams `sh: line 1: spotify-now-playing: command not found` on every `interval` tick. Cause: waybar runs under `systemd.user.services.waybar`, whose unit PATH (see the `Environment=` in `systemctl --user cat waybar.service`) contains only the store coreutils/findutils/grep/sed/systemd dirs — **neither `/run/current-system/sw/bin` nor `/etc/profiles/per-user/<user>/bin`**. A bare `exec = "spotify-now-playing"` / `on-click = "spotify-widget-click"` therefore fails even though the command works in a terminal (the interactive session PATH gets the profile dir from `modules/home/core/nix.nix:7`). Existing modules dodge it with absolute exec paths (`custom/gpu` → `/nix/store/.../bin/amdgpu-stats`); the spotify widget used a bare name and broke. Fix: absolute paths in `exec`/`on-click` — `/etc/profiles/per-user/${flake.config.me.username}/bin/<name>` for home-manager packages (see `configurations/nixos/desktop/default.nix:227`). Latent: pre-existing bare on-clicks (`pavucontrol`, `brightnessctl`, `notify-send` — e.g. `modules/nixos/hyprland/bar/options.nix:37,118`) are equally dead from waybar; the framework fix is to add the user profile to `systemd.user.services.waybar.path`.

**Spotify waybar widget blanks out under API 429s — spotipy retries internally for the whole Retry-After, so the exec process hangs and waybar's interval spawns pile-up instances**

Symptom (2026-09-25, desktop): the now-playing widget stops showing output; `journalctl --user -u waybar` (and the exec's own stdout) shows `Your application has reached a rate/request limit. Retry will occur after: 3232 s`, and `ps` shows several concurrent `now-playing-waybar.py` processes. Cause: the widget calls `sp.current_playback()` (`modules/home/spotify/now-playing-waybar.py:196`) against Spotify's user-scope API (≈180 requests per 30 s window). spotipy 2.26's urllib3 retry config includes 429 in `status_forcelist`, so it retries internally with backoff for the whole `Retry-After` window — printing that message and blocking — before eventually raising `SpotifyException(429)`. The 5 s `interval` fires a new exec while the old one still hangs, so instances pile up and each new one re-hammers the already-limited API; the module shows nothing because the hanging process never completes to emit JSON. Fix: rate-limit guard + resilience added upstream in `spotify-playlist-manager`: `RateLimitGuard` (sliding-window pacing, `Retry-After` cooldown, exponential backoff) integrated into `_ApiProxy._internal_call`, and `watch_now_playing`/`blocks_until_change` now survive 429s by logging `playback.rate_limited`, sleeping one interval, and keeping the last known state instead of treating it as "nothing playing" (commits `3469760`, `31b2585`). The waybar widget in this repo still calls spotipy directly; wiring it through the upstream `PlaylistManager`/guard (as `modules/home/spotify/README.md` Development section intends) is the durable fix.

**jupyter-init-templates hangs every switch — nested non-root blocking `systemctl start` inside an su'd `WantedBy=multi-user.target` oneshot + circular `Requires=` deadlocks the target; user-consumed secrets must match the consuming host's user/group**

Symptom (2026-09-22 laptop): `nix run` fails with `Failed to start transient service unit: Unit nixos-rebuild-switch-to-configuration.service was already loaded or has a fragment file`; `systemctl list-jobs` shows `jupyter-init-templates.service start running` blocking `multi-user.target` → `graphical.target` (and `jupyter-discover.service start waiting`); the stale transient switch unit stays `active (running)` for 18+ min. On the retry, a second failure surfaces: `Activation script snippet 'agenixChown' failed (1) ... chown: invalid user: 'jupyter:jupyter'`. Cause, two parts, both introduced by `feat(jupyter)` (b7502d8): (1) `jupyter-init-templates` is `Type=oneshot`, `RemainAfterExit=true`, `WantedBy=multi-user.target`, and its script runs `su -s bash <user> -c '<init script>'` where the init script ends with a blocking `systemctl start jupyter-discover.service || true`. As the non-root user that call is polkit-denied, and even root's later duplicate start deadlocks because `jupyter-discover` has `requires = [ "jupyter-init-templates" ]` + `after` — its job waits on init-templates which is still `activating` because the script is blocked in that very call: circular wait, no timeout, every subsequent switch hangs. (2) the `jupyter-password` secret in the manifest was owned `jupyter:jupyter` (the module's *defaults*) but laptop sets `user = primary; group = "users"` (the module only creates the dedicated user/group when `cfg.user == "jupyter"`), so agenix `chown` fails — the owner user/group doesn't exist and is not auto-created. Fix (this session): removed the redundant inner `systemctl start` from the su'd script and changed the outer root trigger to `systemctl start --no-block jupyter-discover.service || true` (`modules/nixos/jupyter/config.nix:182` — same `--no-block` pattern as the music-install kick), and changed the manifest owner/group to `seanc:users` (`modules/nixos/secrets/secrets-manifest.json:363`; precedent `github-token-obsidian`, aec7fef). Unblock recipe for a hung switch: `sudo systemctl stop jupyter-init-templates.service` → `systemctl list-jobs` empty → re-run `nix run`. Rules: **never run a blocking `systemctl start` from inside a `WantedBy=multi-user.target` oneshot, and never from an su'd non-root script** — use `--no-block` or a kick unit; **user-consumed secret owners must match the consuming host's user/group, not the module default**.

**`tools/self-improve-commit.sh` allow-list globs are string-compared (quoted RHS) — wildcard entries (skills/commands/AGENT.md RUN LOG) never match; only literal entries like GOTCHAS.md pass**

Symptom (2026-09-22): a grounded self-apply targeting `modules/home/opencode/commands/git-pull-master.md` (RUN LOG lesson: step 2's `git stash push --include-indexed ...` is rejected by current git with `error: unknown option 'include-indexed'` — the staged-only stash flag is `-S`/`--staged`; plain `git stash push -m` covers staged+unstaged alike, so the special-cased second command is unnecessary) was refused by the helper with `REFUSED: target is not on the Decision-2 allow-list (modules/home/opencode/commands/git-pull-master.md)` even though the path matches the documented glob. Cause: `in_allowlist()` in `tools/self-improve-commit.sh:98` compares with `[[ "$p" == "$g" ]]` — the RHS variable is **quoted**, so bash performs a literal string comparison, never a glob match. Every wildcard entry in `ALLOW_GLOBS`/`AGENT_MD_GLOBS` (`modules/home/opencode/skills/*.md`, `modules/home/opencode/commands/*.md`, the `modules/*/AGENT.md` family, …) is dead: only non-wildcard literal entries (`GOTCHAS.md`, `EFFICIENCY-PROPOSALS.md`) can ever match. Consistent with the audit trail: every prior `git log --grep="Self-Improve:"` commit touched only GOTCHAS.md. Fix (needs its own commit — the helper itself is not on the allow-list, so a future agent should take this to the human or fold it into a normal commit): compare with an unquoted glob var (`[[ "$p" == $g ]]`), use `case "$p" in $g)`, or port the fnmatch logic the header claims to mirror from the retired `mcp_server.py`.

**`fetchPnpmDeps` with `fetcherVersion = 3` is rejected on pnpm 11+ — bump to 4 and regenerate the hash**

Symptom (2026-09-22, desktop `nix run`): the system toplevel eval fails with `error: fetchPnpmDeps \`fetcherVersion = 3\` is no longer supported for \`pnpm_11\` or newer. Please upgrade to the latest`. Cause: the pinned nixpkgs now ships pnpm 11, and `pkgs/build-support/node/fetch-pnpm-deps` throws unless `fetcherVersion` is 4 (versions 1–3 removed in the 26.11 release); `modules/nixos/moku.nix` still pinned `fetcherVersion = 3` with a v3 hash. Fix (commit `00c412e`): set `fetcherVersion = 4` and regenerate the hash — build once with a throwaway hash (`sha256-AAAA...`), read the `got:` line from the fixed-output mismatch (`sha256-XuHbezNWyEBzsJeHU4U+eF0m1ecJz0ThZpNBQ6iffxk=`), then commit the real hash. Lesson: any pnpm-based package override in this repo will hit this on the next nixpkgs bump; the v3 → v4 fetcher change also changes the store layout, so the old hash never carries over.

**`permittedInsecurePackages` hardcoded to a ventoy version rots on nixpkgs bumps — derive from `p.name` and mind the `nixpkgs.config` merge semantics**

Symptom (2026-09-22, desktop `nix run`): system toplevel eval fails with `error: Refusing to evaluate package 'ventoy-qt5-1.1.17' … because it is marked as insecure` right after a nixpkgs bump (ventoy 1.1.12 → 1.1.17). Cause, two parts: (1) `modules/nixos/nix.nix` hardcoded `permittedInsecurePackages` entries `ventoy-1.1.12`/`ventoy-qt5-1.1.12`/`ventoy-gtk3-1.1.12` plus a partial `ventoy-1.1.17`, so the actual installed names (`ventoy-qt5-1.1.17`, `ventoy-gtk3-1.1.17`) weren't permitted; the identical stale list in `modules/nixos/ventoy/config.nix` was a red herring during debugging because `nixpkgs.config` merges via `lib.recursiveUpdate` (lists REPLACE, last definition wins) — the `nix.nix` definition entirely shadows the ventoy module's, so editing only the module changed nothing in the merged value. (2) Desktop enables ventoy with default `package = null`, installing ALL four variants (`ventoy`, `ventoy-full`, `ventoy-full-qt`, `ventoy-full-gtk` — pnames `ventoy`/`ventoy-qt5`/`ventoy-gtk3`). Fix (commit `00c412e`): in BOTH `modules/nixos/nix.nix` and `modules/nixos/ventoy/config.nix`, replace the hardcoded version strings with `map (p: p.name) [ ventoy ventoy-full ventoy-full-qt ventoy-full-gtk ]` so the permit list always tracks the pinned nixpkgs. Lesson: never hardcode insecure-package version strings in a persistent config; and when the merged `nixpkgs.config.permittedInsecurePackages` value looks unchanged after editing a module, remember `nixpkgs.config` uses recursiveUpdate — find every defining module before debugging further.

**`git commit` fails with `unable to append to '.git/logs/HEAD': Permission denied` — the reflog is root-owned from an earlier root-run git**

Symptom (2026-09-12): `git commit` aborts with `fatal: cannot update the ref 'HEAD': unable to append to '.git/logs/HEAD': Permission denied`, and `git status` may show the change staged but never committed. Cause: `.git/logs/HEAD` (and `.git/logs/refs/heads/*`) are owned `root:root` mode 644 while `.git` itself is user-owned — almost always because some earlier operation ran git (or a tool that shells out to git) as root in this checkout, so the reflog file kept root ownership. Disabling the reflog does NOT dodge it: git still appends an already-existing reflog file, so `git -c core.logAllRefUpdates=false commit` fails the same way. Fix: `sudo chown "$USER:$(id -gn)" .git/logs/HEAD .git/logs/refs/heads/*` (passwordless sudo on this repo). Alternatively, since the parent `.git/logs` dir is user-owned, the root-owned reflog files can just be `rm`'d — git recreates them on the next ref update (this only loses the reflog, not object history). Verify with `ls -l .git/logs/HEAD`.

**Agent self-improvement collapsed to a single in-band checkpoint (2026-09-12) — gated triage+promote removed; rollback = `git revert 24cce97..HEAD`**

What changed: the gated self-improvement pipeline — the SQLite `learnings`/`review_verdicts`/`fabrication_incidents` tables, the `learning_append`/`learning_promote`/`learning_query`/`learning_review` goals MCP tools, the `learning-promoter` agent, the `triage-review` command, the `scout-skeptical`/`qa-verification`/`adversarial` triage roles, the `triage-capture` plugin, the `opencode-serve.service` + `learning-promoter-watcher.{service,timer}` units, `tools/learning-promoter-launcher.py`, and the four self-improvement model-fallback chains — was decommissioned. It is replaced by a **single, in-band, per-response `SELF_IMPROVE` checkpoint** run by the `build` and `researcher` agents: they capture grounded run lessons and, when a lesson targets an **allow-listed** guidance file (GOTCHAS.md, opencode skill/command RUN LOG sections, module `AGENT.md` RUN LOG sections), apply the append-only edit as their **own** commit through the mechanical commit-helper `tools/self-improve-commit.sh`. That helper enforces (a) evidence `file:line` existence, (b) path allow-list / hard deny-list (`secrets/`, `proxy/`, `disko/`, `network*`, `config.nix`, `options.nix`), (c) append-only diff shape, (d) the Decision-3 rate caps `my.programs.opencode.selfImprove.maxCommitsPerSession/maxCommitsPerDay` (default `null` = uncapped); on any failure it leaves the edit unstaged/uncommitted. Audit trail = `git log --grep="Self-Improve:"` + this file; rollback = `git revert`. The checkpoint + `self-improve-guard` plugin are wired into the base opencode home-module (NOT gated behind `my.programs.goals.enable`), so they run on any host. The `goals` module now only serves the personal/CV DB. Why: the gated two-lineage system was heavy (ensemble triage, DB, headless watcher) and mostly dormant (watcher disabled since 2026-08-25); a single cheap checkpoint with mechanical (non-LLM) guards is simpler and viable on small models.

**Agent self-improvement pipeline decommissioned — deferred DB cleanup on `server` (learning tables no longer used, safe to drop)**

Symptom (2026-09-12): the gated `learnings`/`learning_evidence`/`review_verdicts`/`fabrication_incidents` tables and the `learning_append`/`learning_promote`/`learning_query`/`learning_review` MCP tools were removed as part of the self-improvement simplification (single in-band `SELF_IMPROVE` checkpoint; audit via git log `Self-Improve:` trailer + GOTCHAS/RUN LOG instead). The four learning tables are gone from `modules/home/goals/schema.sql` and `mcp_server.py`, and nothing in the repo reads them anymore. However, the **on-disk goals DB on `server`** (`~/.local/share/goals/goals.db`, created before the migration) still contains whatever orphaned `learnings`/`review_verdicts`/`fabrication_incidents` rows accumulated while the pipeline was dormant (watcher disabled 2026-08-25). Cause: the DB lives only on `server` and `server` was unavailable when this shipped, so the cleanup was deferred. Fix (TODO, for whoever next brings `server` up): the tables are inert — the goals MCP server ignores them after the schema change (it only creates/uses tables defined in the new schema.sql, and does not touch `learnings` etc.). Safest is to drop them once: `sqlite3 ~/.local/share/goals/goals.db "DROP TABLE IF EXISTS review_verdicts; DROP TABLE IF EXISTS fabrication_incidents; DROP TABLE IF EXISTS learning_evidence; DROP TABLE IF EXISTS learnings;"` (back the DB up first). Any `proposed` rows were never acted on under the decommissioned gate, so dropping is lossless. Not blocking; safe to ignore until `server` is back.

**Long oneshot `WantedBy=multi-user.target` services hang every nixos-rebuild switch — keep first-run downloads detached**

Symptom (2026-08-27): `nix run` / `nixos-rebuild switch` fails with `Failed to start transient service unit: Unit nixos-rebuild-switch-to-configuration.service was already loaded or has a fragment file`, and the previous attempt's `switch-to-configuration` shows `active (running)` for 10+ minutes. Cause: `switch-to-configuration switch` finishes with `systemctl start multi-user.target`; when a unit wanted by that target is a long-running `Type=oneshot` (first-run yt-dlp playlist download — `music-install-funky`, hundreds of files, hours), the target's start job waits for it, so the transient activation unit stays started and the NEXT rebuild can't create a unit with the same name. Fix (commit `de042ba`, `modules/nixos/music/config.nix`): yt-dlp installer units are no longer wanted by the target; an instant `music-install-<name>-kick` unit (`wantedBy = [ "multi-user.target" ]`, `Type=oneshot`, `RemainAfterExit = true`) starts them with `systemctl start --no-block`, so boot still installs but never blocks a switch. Direct/FOD rsync installs (seconds) stay inline. To unblock a stuck switch now: `sudo systemctl stop music-install-<name>` (cancels the target's pending job → the transient unit completes and is collected), then re-run the switch. Diagnostic: `systemctl list-jobs` shows `music-install-* start running` + `multi-user.target start waiting`; `systemctl status nixos-rebuild-switch-to-configuration.service --no-pager` for the stuck transient. Audit other `wantedBy = [ "multi-user.target" ]` oneshots for long runtimes before adding more.

**opencode-ensemble is a vendored FORK — never re-add the npm spec, and don't bump it without reading FORK.md**

Symptom: every team *continuation* (wake-lead, member delivery, broadcast, shutdown/stall/chatty/pending-message nudges) runs on the global default model instead of the session's resolved chain — measured on the Aug 25 burst: $3.11 lead session 176/176 continuation turns on `deepseek-v4-flash` (89% of the session), 34% of reviewer sessions flipped mid-session (13.8% of reviewer turns). Cause: upstream `opencode-ensemble` calls `promptAsync({ sessionID, parts })` with **no agent/model** on all ten wake sites; the session silently falls back to the default after turn one — and upstream hasn't fixed it through **0.17.0** (verified 2026-08-26: all sites still absent-model in the latest published dist). Fix (2026-08-26/27): the repo now installs a build-time-patched fork from `https://github.com/Cairnstew/opencode-ensemble` as a *local plugin* (`pluginFiles.opencode-ensemble` → `~/.config/opencode/plugins/opencode-ensemble.js`; derivation `modules/home/opencode/fork.nix`, patch `patches/opencode-ensemble.py`). The npm `plugins` array (`modules/nixos/homeManager/config.nix:171`) must stay EMPTY of it — a similar-named npm spec and local file **BOTH load** (opencode docs) → double-registered tools + the unfixed upstream. Enforced by an assertion in `modules/home/opencode/tests.nix`. Related render gotcha fixed along the way: in `pluginFiles` rendering, `builtins.isPath <derivation>` is `false` (derivations are attrset-valued; `builtins.typeOf` returns `"set"`), so path-sourcing an option value requires `src ? outPath` — otherwise the derivation gets rendered as inline `text` (the config error says `not of type 'null or strings concatenated with "\n"'`). Any ensemble version bump = read `modules/home/opencode/FORK.md` first (re-check upstream, re-base, re-run nixtest + live verification).

**[SUPERSEDED — watcher examples removed 2026-09-12]** **Home-manager-managed live files are read-only `/nix/store` symlinks — hotfix by `rm` + real file, durable fix in the repo**
...
**A boolean option that looks respected but is never read — pacing.enable was a dead flag (2026-08-26)**

Symptom: the entire self-improvement model chain (`learning-promoter`, `scout-skeptical`, `qa-verification`, `adversarial`) resolved BLOCKED (selector exit 5) while the usage cache showed rolling **0–1%** and weekly/monthly well inside every static cap. README and options docs said pacing "ships disabled" (`pacing.enable = false` on every chain entry), yet pace-based rejection was happening anyway. Cause: in `fallback.nix`'s jq resolution program, `paceCap` consulted `entry.pacing.floor`/`entry.pacing.buffer` but **never `entry.pacing.enable`** — the option was declared, documented, serialized into `model-fallback.json`, and then simply not read. The flag's absence made the Aug 31/Sep 19 evidence gates meaningless: they gated a switch that was already effectively ON. Diagnostic signature for this bug class recurring elsewhere: **a selection/cap feature resolves everything to exhausted/ineligible while the cheapest window sits near 0% and all static caps pass** — when static math and observed behavior disagree, suspect an unread flag before suspecting the data. Fix: the pace term is now gated on `(entry.pacing.enable // false)`; the program moved to `modules/home/opencode/model-select.jq` so the nixtest suite (`tests/opencode-model-fallback_test.nix`) runs the exact bytes the selector runs — its `pacing-enable-false-matches-static-only` case fails against the old program by construction. Related hardening from the same fix: interpolating a jq program inside a single-quoted bash argument silently breaks if the text ever contains an apostrophe (caught live: "entry's"), so the selector now loads the program with `jq -f <store-path>` instead; and `--sync-ensemble` now logs a stderr line when it skips a BLOCKED agent (previously stale `modelsByAgent` entries were carried over silently — there is still no fresh-vs-carried marker in the file itself; see `modules/home/opencode/fallback.nix` sync loop).
...
**[SUPERSEDED — triage-capture removed 2026-09-12]** **opencode v1 SDK `session.messages()` doesn't read cross-session — triage-capture reads the `part` table directly**
...
**[SUPERSEDED — watcher timer removed 2026-09-12]** **systemd `OnCalendar` rejects time spans like `"1min"` — use `OnBootSec`/`OnUnitActiveSec` instead**
...
Symptom (2026-08-13): The `servers.<name>.packwiz` path built every mod jar via packwiz2nix (`mkModLinks`) but **nothing else** — a pack shipping `config/<mod>/…` default configs or `config/paxi/datapacks/…` patches had them silently absent from the server's data dir. Cause: the packwiz deploy path only consumed `checksums.json` (mod jars); internal files are a separate packwiz concept that packwiz-installer copies on client install, and our Nix server had no equivalent. Fix: the module symlinks the pack's `kubejs/`, `scripts/`, `datapacks/` subdirs into the data dir at start (`packwizStartPre` in `modules/nixos/minecraft-server/config.nix`, wired into `extraStartPre`) — `mods` is excluded since jars come from checksums.json. Update (2026-08-15): `config/` and `defaultconfigs/` are **not** symlinked — the server writes to them at boot (FML generates `config/fml.toml`, mods write defaults), and a symlink into the read-only store crashes with `Read-only file system` (or `AccessDeniedException` on `config/fml.toml`). They are seeded as real writable dirs with `rsync -a -L --ignore-existing` (pack ships defaults; existing/server-generated files win) + `chmod -R u+w`, the same preserve policy the client instance uses. A stale store symlink from the pre-writable era is removed first (`if [ -L "$d" ]; then rm -f "$d"`). Verify with `mc-pack-status` (it now checks that every on-disk internal file is in index.toml and that Paxi datapack pack_formats match the pack's MC version; 1.21.1 = 48). Related: `config-preserve`/`config-add`/`datapack-add` opencode tools live in `modules/nixos/minecraft-server/opencode/tools/mc-pack.py` — every file edit must `packwiz refresh` or the file is on disk but unindexed (never installed).

---

**A `side = "both"` mod that loads client-only classes crashes the dedicated server — mark it `side = "client"`**

Symptom (2026-08-15): After clearing the client-only render mods (see the Sodium entry above), the DragonTech server crashed again mid-mod-load with `Mod loading has failed` / `Attempted to load class net/minecraft/client/... for invalid dist DEDICATED_SERVER` from FTB Quests Throughput Addon (`throughput`) and Wakes Reforged (`wakes`). Cause: these mods are `side = "both"` in their `.pw.toml` but reference client-only classes (`ClientLevel`, `LocalPlayer`) at mod-load, which NeoForge's RuntimeDistCleaner rejects on a headless server — a *hard* failure, unlike the benign `@Mixin target ... was not found` warnings for client-only mixins (those are safe to ignore). Fix: mark them `side = "client"` in their `.pw.toml` (same as the Sinytra bridge stack) so the server's side filter drops them; the Prism client still gets them. How to find them: after a boot crash, `journalctl -u minecraft-server-<name> | grep -E 'Failed to (create mod instance|register automatic subscribers)\. ModID: \w+'` — those are the fatal ones; `Error loading class ... was not found` mixin warnings are NOT fatal.

---

**The NixOS sudo wrapper is setuid `root:wheel` 4750 — a non-wheel user's NOPASSWD sudoers rule is unreachable until they join `wheel`**

Symptom (2026-08-15): The new dashboard management API (`minecraft-dashboard-api.service`, running as the `minecraft-web` user) reported `PermissionError: [Errno 13] Permission denied: '/run/wrappers/bin/sudo'` when POSTing start/stop/restart, even though `security.sudo.extraRules` grants that user NOPASSWD `systemctl ... minecraft-server-*`. Cause: `/run/wrappers/bin/sudo` is setuid `root:wheel` mode 4750 — only group `wheel` can execute it at all, and `minecraft-web` was only in `minecraft`. The sudoers NOPASSWD rule is irrelevant if the wrapper itself won't run. This also silently broke the web-console dot-commands (`.start`/`.stop`/`.restart` in `mc-web-*`), which shell out to the same `sudo -n systemctl`. Fix: add `extraGroups = [ "wheel" ]` to the web user (`users.users.${cfg.web.user}`); the sudoers rule still scopes exactly which commands are allowed. Also note `systemd.services.<name>.path = [ pkgs.sudo ... ]` must be a **top-level** service attribute, not inside `serviceConfig` (which NixOS ignores for PATH), and the API should call `/run/wrappers/bin/sudo` + `/run/current-system/sw/bin/systemctl` by absolute path since systemd service PATH is minimal.
...
**[SUPERSEDED — /learning-promote + learning-promoter removed 2026-09-12; the `agent:` frontmatter rule still holds]** **Slash-command frontmatter without `agent:` runs as the default primary agent — permission-carrying agent definitions are bypassed**
...
**Waybar 0.11+ battery module renders as an empty/invisible widget — an unknown `{...}` format arg (e.g. the removed `{timeToFull}`) throws in `update()` before the label is set; journal shows `battery: argument not found`**

Symptom (2026-09-22, laptop, waybar 0.15.0): every bar module shows except battery/power. `journalctl --user -u waybar` repeats `[error] battery: argument not found` (interleaved with a bare `Discharging` line — that's waybar's own `puts(status)` debug output) once per poll. Cause: the battery `tooltip-format` contained `{timeToFull}`, which is NOT a documented battery arg in waybar ≥ 0.11 (valid: `{capacity}` `{power}` `{icon}` `{time}` `{timeTo}` `{cycles}` `{health}`). `bar.cpp` wraps `module->update()` in try/catch; `fmt::format` throws on the unknown arg at the tooltip step (`src/modules/battery.cpp:720`) BEFORE `label_.set_markup()` (`battery.cpp:742`), so the label is never set and the module shows as empty. Same mechanism as the disk-module `{used_gb}` outage (see the older waybar crash entry further down) — that one was fixed, the battery one survived. Fix: replaced `{timeToFull}` with `{timeTo}` and modernized the module for 0.15 (status-aware `format-icons` object with charging-bolt/plugged/full, native `events` low-battery notifications, `on-click` lock / `on-click-right` suspend / scroll=backlight via brightnessctl, `smooth-power`). Rule: for waybar ≥ 0.11, only use format args documented in `man waybar-battery(5)` (and per-module man pages); an unknown arg hides the WHOLE module, not just that field — when a waybar module is missing, check `journalctl --user -u waybar` for `argument not found`. See `modules/nixos/hyprland/bar/config.nix`; regression-guarded in `modules/nixos/hyprland/tests.nix`.

---

**The entity-attributes dump (`attributes.py --full-export`) gives EVERY entity all standard combat attributes — cows and chickens have `attack_damage=2`, and `apothic_attributes:*` loads onto every entity — so "has attack_damage" or "has any custom combat attr" cannot distinguish hostile mobs from passives**

Symptom (2026-09-13, building mob-combat.py/mob-tier.py): trying to classify mobs as "passive" by the absence of `attack_damage` (or by the presence of custom combat attributes) mis-classified the whole pack — cows/chickens came out as `full` combat data, and adding `apothic_attributes` to a "combat-overhaul namespaces" set made `_meaningful_custom_combat` return True for every entity, so `combat_data: custom-only` wrongfully swallowed the passive bucket. Cause: the NeoForge attribute dump reflects the full resolved attribute registry — every entity inherits the generic defaults (attack_damage=2, and apothic_attributes registers ~20 passive stats on all 318 entities), and epicfight wires into many vanilla mobs. None of these signals is per-entity discriminating. Fix: passive detection must use the **spawn category** (a mob that only ever spawns as creature/ambient/water is passive) not attribute absence; the "custom-only" (combat driven by custom mechanics) classification must use only `epicfight`/`bettercombat` namespaces (`COMBAT_DRIVEN_NAMESPACES`) with non-default values, never `apothic_attributes` (present on everything); and node-spawn modded entities (boss summons like `epicfight:wither_ghost`) need a distinct combat-capability signal + a recorded `hostility_source` so a reader knows why a neutral-looking registry entry reads hostile or not. Lesson: with registry dumps, "attribute present at a non-trivial value" is not a class discriminator — always cross-reference spawn/category data and separate factual completeness from hostility.
...
**`ci-monitor watch` returns instantly instead of blocking — invalid `--exit-name` flag silently fails**

Symptom (2026-09-22): `ci-monitor watch` (the shell script in `packages/ci-monitor/default.nix`) exits immediately without waiting for the CI run to complete, because `gh run watch` rejects the unknown `--exit-name complete` flag. The `if !` on line 244 catches the error as "run failed/cancelled" and proceeds to fetch details, but the watch never actually blocked. Root cause: `--exit-name` is not a valid `gh run watch` flag — only `--exit-status` exists (confirmed: `gh run watch --help` in gh v2.100.0). The TypeScript tool version (`modules/home/opencode/tools/ci-monitor.ts:295`) correctly uses only `--exit-status`. Fix (this session): removed `--exit-name complete` from line 244. Lesson: `gh run watch` has exactly one exit-related flag (`--exit-status`); any other flag starting with `--exit-` is invalid and will cause a silent early exit when wrapped in `if !`.
...
**systemd timer fails with "Unit X.service not found" at activation when a `requires`-related service is gated off**

Symptom (2026-09-21): `nixos-rebuild switch` on laptop (and desktop) reports `warning: the following units failed: suwayomi-sync-export.timer` → `Active: failed (Result: resources)` with `Failed to queue unit startup job: Unit suwayomi-server.service not found.` and the switch returns exit status 4. Cause: `modules/nixos/suwayomi/sync.nix` installed the export service+timer under `lib.mkIf cfg.sync.export.enable` (default true) alone, but the service declares `requires = [ "suwayomi-server.service" ]` (`sync.nix:103`). When the Suwayomi server itself is disabled (`my.services.suwayomi.enable = false`, the default on laptop/desktop) the dependency unit does not exist, and systemd refuses to queue the timer's activation job — every switch fails, not just the once-a-day timer run. The sibling `suwayomi-sync-import` module had the correct guard (`cfg.enable && importCfg.enable`); `sync.nix` was missing the `cfg.enable` half. Fix: gate the export unit on `lib.mkIf (cfg.enable && serverCfg.enable)` in `sync.nix:99`. General rule: whenever a systemd unit declares `requires`/`after` on another unit, both sides must be gated by the same `enable` — a unit that `requires` a non-existent unit fails the entire activation.

---

**git workflow: `nixpkgs-fmt`/`statix`/`deadnix` are not on the host PATH — wrap every one in `nix shell nixpkgs#<pkg> -c`**

Symptom (2026-09-22, laptop git-commit workflow): the git-workflow steps in `agents/build.md` say to run `nixpkgs-fmt --check <files>` and `statix check .` / `deadnix` directly. On laptop (and this repo's hosts generally) none of these binaries are on PATH: `nixpkgs-fmt` fails with `No such file or directory (os error 2)`, and statix/deadnix the same. Cause: the repo's formatter (`nix fmt`) is available as a flake output, but the individual binaries are not installed into the profile. Fix: invoke each via `nix shell nixpkgs#nixpkgs-fmt -c nixpkgs-fmt --check <files>` (and `nix shell nixpkgs#statix nixpkgs#deadnix -c sh -c 'statix check . && deadnix --no-lambda-pattern-names .'`); the cached binary builds make these ~1s after first fetch. Note statix takes a single TARGET arg, not multiple files (`statix check f1 f2` errors with "unexpected argument"), so run it per-directory or on `.`. Lesson: treat the git-workflow validation commands as flake-wrapped, never bare.

---

**CI auto-merge divergence: `git pull --rebase origin <branch>` is NOT enough when master has conflicting edits — rebase onto `origin/master` instead**

Symptom (2026-09-22, laptop): after the usual `git pull --rebase origin laptop` and push, the `Merge Per-Host Branch` workflow failed (`auto-merge: failure`, run 35778053740) with real content conflicts — `GOTCHAS.md` and `modules/nixos/secrets/secrets-manifest.json` — because master had meanwhile gained 20+ commits (desktop CI merges) that edited the *same lines* the laptop branch was changing (jupyter-password owner `jupyter`→`seanc`, GOTCHAS tail). The workflow's "rebase first" advice only syncs you with origin/<branch>, which is stale relative to master once other hosts auto-merge. Fix (worked this run): `git rebase origin/master` on the host branch — master is fully contained afterward (`git rev-list --count laptop..master` = 0), so the auto-merge becomes clean. The rebase hit several conflicts; the opencode budget-pacing commits (`add budget pacing`, `wire slice roll`, `harden degrade path`) were *duplicates already on master via desktop's lineage* — verified by checking master had the same files/content, then `git rebase --skip` each; genuine laptop-only commits (eduroam, suwayomi, my jupyter/waybar fixes) were kept and their GOTCHAS entries merged by hand. Push the rewritten branch with `git push --force-with-lease` (history changed). Lesson: if `merge-per-host` CI ever fails on a host branch, check `git rev-list --count <branch>..origin/master` — if master has commits the branch lacks, rebase onto master, skip already-merged duplicates, force-with-lease push.

---

**`nixpkgs-fmt` mangles `.md` command/skill files too — never run `nix fmt -- <non-nix-file>` on opencode docs**

Symptom (2026-09-23, editing `git-commit-push`): verifying formatting of `modules/home/opencode/commands/git-commit-push.md` by running `nix fmt -- <file>` reported "1/1 have been reformatted" and silently rewrote the markdown — nixpkgs-fmt parsed it as Nix and stripped the 2-space indent inside a bash fenced block (`nixpkgs-fmt --check` line), which I had to restore by hand. Cause: `nix fmt -- <path>` hands the file to nixpkgs-fmt regardless of extension, exactly like the `.json` case at GOTCHAS.md:535. Fix: never pass a non-`.nix` file to `nix fmt` (or `nixpkgs-fmt`) directly. For markdown docs, just eyeball/`git diff` the whitespace — and never let a "reformat" claim on an `.md`/`.json` file be accepted for git add; inspect and revert it first (`git checkout -- <file>`).

---

**`nix flake check` does NOT run nixtests — a vendored-fork/regression suite can rot silently for weeks**

Symptom (2026-09-25, space-discovery hardening): `nix flake check --no-build` reported `all checks passed!` while `tests/opencode-ensemble-fork_test.nix` was failing — the fork test asserted 11 `__ensembleWakeArgs(` wake sites, but the actual patched bundle (fork.nix building from `vendor/opencode-ensemble-0.19.0-dist.js`) produces 5. Cause: nixtests are wired as a `packages.nixtests`/`nixtests-run` APP (`modules/flake-parts/nixtest.nix:41-44`), and `nix flake check` evaluates derivations but never RUNS the app — the suite is only exercized by an explicit `nix run .#nixtests-run`. The stale expectations (11 sites → 5, 1 bare `promptAsync({` → 8, `ADD COLUMN lead_model` → helper-only references, `"snapshot the lead"` marker → `ALTER TABLE team ADD COLUMN lead_agent TEXT`) had rotted since commit `dd82ef1` (fd5555e Agent Spaces expansion) with zero signal — verified pre-existing by running the patch against the pristine HEAD dist (`git show HEAD:...dist.js` → `python3 patches/opencode-ensemble.py` → still 5, not 11). Fix (this session): `tests/opencode-ensemble-fork_test.nix:37,47,56-57` now assert the CURRENT fork-generation invariants (5 wake sites, 8 bare sites with agent/model threaded by the expansion, `lead_model` helper refs, lead_agent migration) + a NEW `tests/opencode-ensemble-vendor_test.nix` hash-guard pins the exact vendored bytes. Lesson: after ANY change to a vendored artifact, a fork patch, or the fork version, run `nix run .#nixtests-run` EXPLICITLY — a green `nix flake check` proves nothing about the test suites. To scope to one suite while iterating: `nix run .#nixtests-run -- -s '<other-suites-regex>'` (nixtest has `-s/--skip`, no positive filter).
